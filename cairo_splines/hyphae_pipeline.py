...
def run(d, max_frames=constants.MAX_GROWTH_STEPS):
    logging.info("Running Calculation")
    frame_index = 0
    while d.filter_frontier_to_boundary() and frame_index < max_frames:
        if frame_index % 10 == 0:
            logging.info("Frame: {}".format(frame_index))
            d.inc_time_steps()
        if d.debug_flag:
            d.draw_instance.draw()
            d.draw_instance.write_file(frame_index)
